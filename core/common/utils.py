...
from core.classes.exceptions import InvalidContentError, InvalidDataError, InvalidImageError
from core.classes.http import RosHTTP
from core.classes.memory import Memory
from core.common.config import BANNER, EXPR_ACCT_TICKET, EXPR_SESS_KEY, EXPR_SESS_TICKET, FLASK_DEBUG, FLASK_FOLDER, FLASK_HOST, FLASK_PORT, PATTERN, VERSION
...
def _get_data_len(data: dict[str] | bytes) -> bytes:
    ...
    match len(data_len):
        ...
        case _:
            raise InvalidImageError if image else InvalidDataError
...
def app_clone() -> Response:
    match request.json:
        case {'id': id, 'name': name, 'desc': desc, 'tags': tags, 'data': data, 'lang': lang, 'image': image}:
            ...
            elif '<Status>1</Status>' in response:
